...
export default async function LoginPage({
  ...
  if (params.error === "auth") {
    errorMessage = "No se pudo completar el inicio de sesión. Inténtalo de nuevo.";
  } else if (params.error === "config") {
    errorMessage = "Autenticación no configurada correctamente.";
  }
  ...
  return (
    <div className="min-h-dvh bg-background flex flex-col">
        ...
        <div className="w-full max-w-md space-y-8">
          <div className="text-center space-y-2">
            ...
            <p className="text-base text-muted-foreground">
              Accede con tu correo y contraseña o crea una cuenta.
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 pt-4">
            <div className="flex flex-col items-center text-center gap-2">
              ...
              <span className="text-xs font-medium text-muted-foreground">Mapa</span>
            </div>
            <div className="flex flex-col items-center text-center gap-2">
      ...
      <footer className="py-6 text-center pb-safe">
        <p className="text-xs text-muted-foreground">{tenant.appName}</p>
        <p className="text-xs text-muted-foreground/60 mt-1">{tenant.supportEmail}</p>
      </footer>
    </div>
  );
}
